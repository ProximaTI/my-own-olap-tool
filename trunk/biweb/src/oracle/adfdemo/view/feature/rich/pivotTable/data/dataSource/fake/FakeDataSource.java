/* $Header: dsstools/modules/dvt-faces-demo/src/oracle/adfdemo/view/feature/rich/pivotTable/data/dataSource/fake/FakeDataSource.java /main/4 2009/03/21 00:59:35 ahadi Exp $ */
...
/**
 *  @version $Header: dsstools/modules/dvt-faces-demo/src/oracle/adfdemo/view/feature/rich/pivotTable/data/dataSource/fake/FakeDataSource.java /main/4 2009/03/21 00:59:35 ahadi Exp $
 *  @author  dahmed  
...
{                           // LocalDataSource
    ...
    private void checkLayer(int edge, int layer) throws LayerOutOfRangeException {
        if (layer<0 || layer>=layerCount[edge]) throw new LayerOutOfRangeException(layer, layerCount[edge] - 1);
    }
    ...
    public int[] getSlicesFromQDR(QDRInterface qdr, int[] startSlices, int[] endSlices) {
        ...
        for (Map.Entry<String, Object> entry : sortedMap.entrySet()) {
            ...
            if (edge < 0 || edge > 1 || 
                layer < 0 || layer >= _getLayerCount(edge) || layer != _getMemberStartLayer(edge, layer, slice) ||
                slice < 0 || slice >= _getEdgeExtent(edge) || slice != _getMemberStartSlice(edge, layer, slice)) 
            {
                return new int[] { -1, -1 };
            }
            ...
            // following the previous (parent) member, and that the slice is consistent with being a child of the parent
                ...
                if (parentLayer + parentDepth != layer || 
                    slice < parentSlice ||
                    slice >= parentSlice + parentExtent)
                {
                    return new int[] { -1, -1 };
                }
            }
