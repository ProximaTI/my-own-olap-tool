/* $Header: dsstools/modules/dvt-faces-demo/src/oracle/adfdemo/view/feature/rich/pivotTable/data/dataSource/rowset/RowsetDataSource.java /main/2 2009/03/19 21:37:43 teclee Exp $ */
...
/**
 *  @version $Header: dsstools/modules/dvt-faces-demo/src/oracle/adfdemo/view/feature/rich/pivotTable/data/dataSource/rowset/RowsetDataSource.java /main/2 2009/03/19 21:37:43 teclee Exp $
 *  @author  dahmed
 *  @since   release specific (what release of product did this appear in)
 */
public class RowsetDataSource extends DataAccessAdapter implements CubeDataDirector,
                                                                   CubeDataAccess,
    ...
    public Object[][] fetchData() {
        ...
        if (m_data instanceof CollectionModel) {
            ...
            context.getExternalContext().getRequestMap().put("dvt_sample_collectionModel",
                                                             model);
            ...
            for (int r = 0; r < rowCount; r++) {
                ...
                for (int c = 0; c < colCount; c++) {
                    String elExpr =
                        "#{request.dvt_sample_collectionModel." + m_columns[c] +
                        "}";
                    row[c] =
        ...
        if (rows.size() == 0) {
            ...
            context.addMessage(null,
                               new FacesMessage(FacesMessage.SEVERITY_INFO,
                                                "No data selected.  Please modify the rowset datasource filters in order to display more data.",
                                                ""));
        }
    ...
    protected boolean filterRow(HashMap<String, FilterSpec> filterSpecs,
                                Object[] row) {
        ...
        try {
            if (m_da == null) {
                ...
                if (m_colSectionLayout != null)
                    colSectionLayout =
                            m_colSectionLayout.toArray(new String[0]);
                ...
                if ((rowSectionLayout != null) && (colSectionLayout != null))
                    layout =
                            new String[][] { m_colLayout.toArray(new String[0]),
                ...
                HashMap<String, Double> dataAttributeMinimums =
                    new HashMap<String, Double>();
                HashMap<String, Double> dataAttributeMaximums =
                    new HashMap<String, Double>();
                ...
                for (int i = 0; i < m_dataAttributes.size(); i++) {
                    ...
                    if (filter != null) {
                        ...
                        if (filter.isDataEnabled()) {
                            dataAttributeMinimums.put(dataAttribute,
                                                      filter.getMin().doubleValue());
                            dataAttributeMaximums.put(dataAttribute,
                                                      filter.getMax().doubleValue());
                ...
                RowProjection proj =
                    new BaseProjectionImpl(layout, dataAttributes.toArray(new String[0]),
                                           data, m_columns);
                AggSpec[][] aggSpecs = null;
                if (isTotalsEnabled())
                    aggSpecs = getAggSpecs();
                RowBasedCubicDataAccess cubicDA =
                    new RowBasedCubicDataAccess(new ResultTable(proj,
                                                                aggSpecs));
                m_cubicDA = cubicDA;
                // wrap the data access with a decorator, so that we can add values to
                // DataAccess
                m_da =
new RowsetDataAccessDecorator(cubicDA, getFilterSpecs(), m_updatedValues);
                m_isDirty = false;
            }
            return m_da;
        } catch (Exception e) {
    ...
    public boolean updateMatch(QDR qdr, Object[] row, String[] columns,
                               String dataAttribute, Object newValue) {
    ...
    public boolean valueChanged(QDR qdr, Object newObjectValue,
        ...
        m_isDirty =
                true; // mark the DataAccess for update on the next sort, since sort doesn't typically reload the rowset
        if (newObjectValue != null) {
            ...
            if (m_data instanceof Object[][]) {
                ...
                for (int i = 0; i < data.length; i++) {
                    Object[] row = data[i];
                    if (updateMatch(qdr, row, m_columns, dataAttribute,
                                    newObjectValue))
                        break;
                }
            }
            ...
            if (newObjectValue instanceof Number) {
                ...
                // update the limits
                Iterator<FilterSpec> iterator =
                    getFilterSpecs().values().iterator();
                while (iterator.hasNext()) {
                    FilterSpec filter = iterator.next();
                        ...
                        if (_newValue > filter.getMax().longValue()) {
                            ...
                            if (oldValue != null) {
                                ...
                                if (_oldValue == filter.getMax().longValue()) {
                                    ...
                                    long newMax =
                                        computeDataAttributeMaxValue(qdr,
                                                                     dataAttribute,
                                                                     _newValue);
    ...
    protected long computeDataAttributeMaxValue(QDR updatingQDR,
                                                String dataAttribute,
                                                long newValue) {
        return computeDataAttributeLimit(updatingQDR, dataAttribute, newValue,
                                         true);
    }
    ...
    protected long computeDataAttributeMinValue(QDR updatingQDR,
                                                String dataAttribute,
                                                long newValue) {
        return computeDataAttributeLimit(updatingQDR, dataAttribute, newValue,
                                         false);
    ...
    protected long computeDataAttributeLimit(QDR updatingQDR,
                                             String dataAttribute,
                                             long newValue, boolean isMax) {
    ...
    public boolean setSorts(SortInfo[] sortInfo) throws LayerOutOfRangeException,
                            ...
                            if (slice != -1) {
                                try {
                                    if (m_isDirty) {
                                        ...
                                        return m_cubicDA.sort(edge, slice,
                                                              direction,
                                                              sliceInfo.isNullsFirst(),
                                                              sliceInfo.isGrouped());
                                } catch (DataException e) {
                                    throw new DataDirectorException("Sort Failed",
                                                                    e);
                                }
                            }
                        }
    ...
    public boolean pivotOK(int fromEdge, int toEdge, int fromLayer,
                           int toLayer,
                           int flags) throws EdgeOutOfRangeException,
                                             LayerOutOfRangeException,
                                             DataDirectorException {
    ...
    public void addDataDirectorListener(DataDirectorListener l) {
        listener = l;
        l.viewDataAvailable(new DataAvailableEvent(this,
                                                   this.getDataAccess()));
    }
    ...
    public MetadataMap getMetadataMap(int edge,
                                      int layer) throws EdgeOutOfRangeException,
                                                        LayerOutOfRangeException {
    ...
    public MetadataMap getSupportedMetadataMap() {
        String supported[] =
        { MetadataMap.METADATA_LONGLABEL, MetadataMap.METADATA_DRILLSTATE };
